...
const load = async () => {
  ...
  const byParty = new Map<string, { name: string; ap: number; freight: number }>()
  for (const r of (data ?? []) as any[]) {
    const key = r.party_id
    const entry = byParty.get(key) ?? { name: r.parties?.name || '—', ap: 0, freight: 0 }
    const net = Number(r.credit || 0) - Number(r.debit || 0)
    if (r.accounts?.code === '2100') entry.ap += net
    else entry.freight += net
    byParty.set(key, entry)
  }
  rows.value = [...byParty.values()]
    .map((r) => ({ ...r, total: r.ap + r.freight }))
  ...
  <div class="print-root">
    ...
    <div v-else class="sheet" :lang="printLocale">
      <div class="letterhead">
        ...
        <div class="title">{{ t('printGov.payables.title') }}</div>
        <div class="small">{{ t('printGov.payables.as_of', { date: today }) }}</div>
      </div>

      <table class="lines">
        <thead>
          <tr>
            <th>{{ t('printGov.payables.col_party') }}</th>
            <th class="num">{{ t('printGov.payables.col_ap') }}</th>
            <th class="num">{{ t('printGov.payables.col_freight') }}</th>
            <th class="num">{{ t('printGov.payables.col_total') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="!rows.length"><td colspan="4" class="empty">{{ t('printGov.payables.empty') }}</td></tr>
          <tr v-for="(r, i) in rows" :key="i">
            <td>{{ r.name }}</td>
            <td class="num">{{ Number(r.ap).toLocaleString('en-IN') }}</td>
            <td class="num">{{ Number(r.freight).toLocaleString('en-IN') }}</td>
            <td class="num">{{ Number(r.total).toLocaleString('en-IN') }}</td>
          </tr>
          <tr v-if="rows.length" class="total-row">
            <td colspan="3">{{ t('printGov.payables.grand_total') }}</td>
