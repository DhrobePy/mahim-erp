...
const categoryLabel = computed<Record<string, string>>(() => ({
  business_capital: t('printGov.it10b.categories.business_capital'),
  non_agri_property: t('printGov.it10b.categories.non_agri_property'),
  agri_property: t('printGov.it10b.categories.agri_property'),
  investments: t('printGov.it10b.categories.investments'),
  motor_vehicles: t('printGov.it10b.categories.motor_vehicles'),
  ornaments: t('printGov.it10b.categories.ornaments'),
  furniture_electronics: t('printGov.it10b.categories.furniture_electronics'),
  cash_bank: t('printGov.it10b.categories.cash_bank'),
  other_assets: t('printGov.it10b.categories.other_assets'),
  mortgage_liability: t('printGov.it10b.categories.mortgage_liability'),
  bank_loan_liability: t('printGov.it10b.categories.bank_loan_liability'),
  other_liability: t('printGov.it10b.categories.other_liability')
}))
...
<template>
  <div class="print-root">
    <div class="no-print toolbar">
      <NuxtLink to="/admin/tax" class="back">{{ t('printGov.it10b.back') }}</NuxtLink>
    ...
    <div v-else-if="statement" class="sheet" :lang="printLocale">
      <p class="form-tag">{{ t('printGov.it10b.form_tag') }}</p>
      <div class="doc-title">{{ t('printGov.it10b.doc_title') }}</div>
      <p class="small center">{{ t('printGov.it10b.subtitle', { date: fmtDate(statement.statement_date), year: statement.assessment_year }) }}</p>

      <table class="meta">
        <tbody>
          <tr>
            <td>
              <div class="small">{{ t('printGov.it10b.name_of_assessee') }}</div>
              <b>{{ who }}</b>
              <div class="small">{{ statement.company_directors?.designation ? statement.company_directors.designation.replace('_', ' ') : '' }}</div>
            </td>
            <td>
              <div class="small">{{ t('printGov.it10b.tin') }}</div>
              <b class="mono">{{ statement.individual_tin || statement.company_directors?.tin_no || '—' }}</b>
              <div class="small mt">{{ t('printGov.it10b.nid') }}</div>
              <b class="mono">{{ statement.company_directors?.nid_no || '—' }}</b>
            </td>
            <td>
              <div class="small">{{ t('printGov.it10b.address') }}</div>
      ...
      <p class="section-hdr">{{ t('printGov.it10b.section_a') }}</p>
      <table class="lines">
        <thead><tr><th style="width:24px;">{{ t('printGov.it10b.col_sl') }}</th><th>{{ t('printGov.it10b.col_head_asset') }}</th><th>{{ t('printGov.it10b.col_description') }}</th><th class="right">{{ t('printGov.it10b.col_amount') }}</th></tr></thead>
        <tbody>
          ...
          <tr v-if="!linesFor(assetCats).length"><td colspan="4" class="small center">{{ t('printGov.it10b.no_asset_lines') }}</td></tr>
          <tr class="total-row"><td colspan="3"><b>{{ t('printGov.it10b.total_assets') }}</b></td><td class="right mono"><b>{{ num(totalAssets) }}</b></td></tr>
        </tbody>
      </table>

      <p class="section-hdr">{{ t('printGov.it10b.section_b') }}</p>
      <table class="lines">
        <thead><tr><th style="width:24px;">{{ t('printGov.it10b.col_sl') }}</th><th>{{ t('printGov.it10b.col_head_liability') }}</th><th>{{ t('printGov.it10b.col_description') }}</th><th class="right">{{ t('printGov.it10b.col_amount') }}</th></tr></thead>
        <tbody>
          <tr v-for="(l, i) in linesFor(liabilityCats)" :key="l.id">
            <td>{{ i + 1 }}</td><td>{{ categoryLabel[l.category] }}</td><td>{{ l.description }}</td>
            <td class="right mono">{{ num(l.amount) }}</td>
          </tr>
          <tr v-if="!linesFor(liabilityCats).length"><td colspan="4" class="small center">{{ t('printGov.it10b.no_liability_lines') }}</td></tr>
          <tr class="total-row"><td colspan="3"><b>{{ t('printGov.it10b.total_liabilities') }}</b></td><td class="right mono"><b>{{ num(totalLiabilities) }}</b></td></tr>
        </tbody>
      </table>

      <table class="lines">
        <tbody>
          <tr class="total-row"><td><b>{{ t('printGov.it10b.net_wealth') }}</b></td><td class="right mono"><b>{{ num(netWealth) }}</b></td></tr>
        </tbody>
      </table>
      <p class="small">{{ t('printGov.it10b.in_words', { words: takaWords(netWealth) }) }}</p>

      <p class="section-hdr">{{ t('printGov.it10b.section_c') }}</p>
      <table class="lines">
        <tbody>
          <tr><td>{{ t('printGov.it10b.opening_net_wealth') }}</td><td class="right mono">{{ num(statement.opening_net_wealth) }}</td></tr>
          <tr><td>{{ t('printGov.it10b.add_total_income') }}</td><td class="right mono">{{ num(statement.total_income) }}</td></tr>
          <tr><td>{{ t('printGov.it10b.less_total_expenditure') }}</td><td class="right mono">({{ num(statement.total_expenditure) }})</td></tr>
          <tr class="total-row"><td><b>{{ t('printGov.it10b.closing_net_wealth') }}</b></td><td class="right mono"><b>{{ num(closingCheck) }}</b></td></tr>
          <tr>
            <td><b>{{ t('printGov.it10b.net_wealth_check') }}</b></td>
      ...
      <p v-if="Math.abs(closingCheck - netWealth) > 1" class="small mismatch">
        {{ t('printGov.it10b.mismatch_warning') }}
      </p>

      <p class="disclaimer">
        {{ t('printGov.it10b.disclaimer') }}
      </p>

      <div class="row spread sig-block">
        <div class="sig"><div class="sig-line" /><div class="small">{{ t('printGov.it10b.sig_assessee') }}</div></div>
        <div class="sig"><div class="sig-line" /><div class="small">{{ t('printGov.it10b.sig_date') }}</div></div>
      </div>
    </div>
  </div>
</template>
